...
class PostgresConnector:
    ...
    def _query(self, last_pk: object) -> str:
        ...
        columns = sorted(
            {self.pk_field, self.title_field, self.updated_at_field, *self.content_fields}
        )
        ...
        return (
            f"SELECT {selected} FROM {safe_table}{where} "
            f"ORDER BY {safe_pk} LIMIT {self.batch_size}"
        )
